 ...
 *           REMAP_SIGN_P7]
 */
void Jims_BNO055::setAxisSign(adafruit_bno055_axis_remap_sign_t remapsign) {
  adafruit_bno055_opmode_t modeback = mMode;
 ...
 *           system error info
 */
void Jims_BNO055::getSystemStatus(uint8_t *system_status,
 ...
 *          revision info
 */
void Jims_BNO055::getRevInfo(adafruit_bno055_rev_info_t *info) {
  uint8_t a, b;
 ...
 */
imu::Vector<3> Jims_BNO055::getVector(adafruit_vector_type_t vector_type) {
  imu::Vector<3> xyz;
  uint8_t buffer[6];
  memset(buffer, 0, 6);
 ...
 */
imu::Quaternion Jims_BNO055::getQuat() {
  uint8_t buffer[8];
  memset(buffer, 0, 8);
 ...
 *          Sensor description
 */
void Jims_BNO055::getSensor(sensor_t *sensor) {
 ...
 */
bool Jims_BNO055::getEvent(sensors_event_t *event) {
  /* Clear the event */
  memset(event, 0, sizeof(sensors_event_t));

  event->version = sizeof(sensors_event_t);
  event->sensor_id = mSensorID;
  event->type = SENSOR_TYPE_ORIENTATION;
  event->timestamp = millis();

  /* Get a Euler angle sample for orientation */
  imu::Vector<3> euler = getVector(Jims_BNO055::VECTOR_EULER);
...
bool Jims_BNO055::getEvent(sensors_event_t *event,
  ...
  imu::Vector<3> vec;
  if (vec_type == Jims_BNO055::VECTOR_LINEARACCEL) {
    event->type = SENSOR_TYPE_LINEAR_ACCELERATION;
    vec = getVector(Jims_BNO055::VECTOR_LINEARACCEL);

    event->acceleration.x = vec.x();
    event->acceleration.y = vec.y();
    event->acceleration.z = vec.z();
  } else if (vec_type == Jims_BNO055::VECTOR_ACCELEROMETER) {
    event->type = SENSOR_TYPE_ACCELEROMETER;
    vec = getVector(Jims_BNO055::VECTOR_ACCELEROMETER);

    event->acceleration.x = vec.x();
    event->acceleration.y = vec.y();
    event->acceleration.z = vec.z();
  } else if (vec_type == Jims_BNO055::VECTOR_GRAVITY) {
    event->type = SENSOR_TYPE_GRAVITY;
    vec = getVector(Jims_BNO055::VECTOR_GRAVITY);

    event->acceleration.x = vec.x();
    event->acceleration.y = vec.y();
    event->acceleration.z = vec.z();
  } else if (vec_type == Jims_BNO055::VECTOR_EULER) {
    event->type = SENSOR_TYPE_ORIENTATION;
    vec = getVector(Jims_BNO055::VECTOR_EULER);

    event->orientation.x = vec.x();
    event->orientation.y = vec.y();
    event->orientation.z = vec.z();
  } else if (vec_type == Jims_BNO055::VECTOR_GYROSCOPE) {
    event->type = SENSOR_TYPE_GYROSCOPE;
    vec = getVector(Jims_BNO055::VECTOR_GYROSCOPE);

    event->gyro.x = vec.x() * SENSORS_DPS_TO_RADS;
    event->gyro.y = vec.y() * SENSORS_DPS_TO_RADS;
    event->gyro.z = vec.z() * SENSORS_DPS_TO_RADS;
  } else if (vec_type == Jims_BNO055::VECTOR_MAGNETOMETER) {
    event->type = SENSOR_TYPE_MAGNETIC_FIELD;
    vec = getVector(Jims_BNO055::VECTOR_MAGNETOMETER);
 ...
 */
bool Jims_BNO055::getSensorOffsets(uint8_t *calibData) {
  if (isFullyCalibrated()) {
    adafruit_bno055_opmode_t lastMode = mMode;
    setMode(OPERATION_MODE_CONFIG);
 ...
 *  @return true if read is successful
 */
bool Jims_BNO055::getSensorOffsets(
 ...
 *          calibration data
 */
void Jims_BNO055::setSensorOffsets(const uint8_t *calibData) {
  adafruit_bno055_opmode_t lastMode = mMode;
  setMode(OPERATION_MODE_CONFIG);
  sleep_for(milliseconds(25));
 ...
 */
void Jims_BNO055::setSensorOffsets(
